...
int main()
{
  ...
  */
  ...
  for(int i = 0; i < N-1; i++)
  {
    diag_a[i] = (-1) * (h / 2) * p_function(malha_x[i + 1]) - 1;

    diag_b[i] = 2 + h * h * q_function(malha_x[i + 1]);

    diag_c[i] = (h / 2) * p_function(malha_x[i + 1]) - 1;
  }
  ...
  for(int i = 1; i < N; i++)
    malha_v[i] = -1*pow(h,2)*r_function(malha_x[i]);

  malha_v[1] += (1 + (h/2)*p_function(malha_x[0]))*sol_a;
  malha_v[N-1] += (1 + (h/2)*p_function(malha_x[N]))*sol_b;
  ...
  for (int i = 0; i <= N; i++)
  {
    ...
    cout << malha_x[i] << " | " << malha_w[i] << " | " << valor_real << " | " << malha_w[i] - valor_real << endl;
  }
