  ...
  */
  ...
  for(int i = 1; i <= M; i++){
    for (int j = 1; j <= N; j++){
      ...
      diag[4][k] = (j < M) ? -K_half(Ky[k], Ky[k + M]) / hy2 : 0.0;
...
|             . . 4 . . . ]           |       |       ]
  ...
  */
  ...
  for (int i = 2; i <= M; i++)
  {
    ...
    LUMatrix[4][i] = diag[4][i] / LUMatrix[2][i];
  ...
  */
  ...
  for(int i = 1; i <= M; i++){
    for (int j = 1; j <= N; j++){
      ...
      campoVel[2][k] += (i > 1) ? -(K_half(Kx[k], Kx[k - 1]) * (valor_real[k] - valor_real[k-1])) / (hx * 2) : 0.0;
      campoVel[2][k] += (i < M) ? -(K_half(Kx[k], Kx[k + 1]) * (valor_real[k+1] - valor_real[k])) / (hx * 2) : 0.0;

      // Passo 6.2 >> Norte + Sul
      campoVel[1][k] += (j > 1) ? -(K_half(Ky[k], Ky[k - M]) * (w[k] - w[k-M])) / (hy * 2) : 0.0;
      campoVel[1][k] += (j < N) ? -(K_half(Ky[k], Ky[k + 1]) * (w[k+M] - w[k])) / (hy * 2) : 0.0;

      campoVel[3][k] += (j > 1) ? -(K_half(Ky[k], Ky[k - M]) * (valor_real[k] - valor_real[k-M])) / (hy * 2) : 0.0;
      campoVel[3][k] += (j < N) ? -(K_half(Ky[k], Ky[k + 1]) * (valor_real[k+M] - valor_real[k])) / (hy * 2) : 0.0;      
    }
  }
