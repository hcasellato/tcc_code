...
void LU_solve(double **D, double *X, double *F, double **L, double **U){
  double sum;
...
void FinVol(int debug_const)
{
  ...
  */
  ...
  for(int i = 1; i <= TB; i++){
    ...
    LU_solve(Atil[1], G[1][i], C[i], L, U);
  }

  // Passo 4.1 >> Ã1 * z1 = d1 => z1
  LU_solve(Atil[1], Z[1], b_d[1], L, U);
...
// g++ 2D_EDP_BLUv5.cpp -lm -o 2D_EDP_BLUv5 && ./2D_EDP_BLUv5