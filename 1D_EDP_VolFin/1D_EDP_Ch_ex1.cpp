...
//
// Tenta a implementação de Fatoração de Cholensky, especializando para matrizes tridiagonais para os problemas
// unidimensionais do livro de Volumes Finitos. Lembrando que só funciona (ao contrário de CF), pois a matriz A
// é simétrica.
//
// Problemas da forma: 
//
...
void FinVol(int N)
{
  ...
  */
  ...
  // Passos 4-9
  double ll[N + 1], ld[N + 1], z[N + 1], w[N + 2];
  ...
  // Não vou usar, pois a matriz é SPD
  ...
  for(int i = 2; i < N; i++)
  {
    ld[i]   = sqrt(a[i] - ll[i]*ll[i]);
    ll[i+1] = c[i+1] / ld[i];

    z[i] = (d[i] - (ll[i] * z[i-1])) / ld[i];
  }
  
  // Passo 6
  ld[N] = sqrt(a[N] - ll[N]*ll[N]);
  z[N] = (d[N] - (ll[N] * z[N-1])) / ld[N];
  ...
  ofstream file;
  string name_file = "1D_finVolMet_ex1/1D_finVolMet_Ch_ex1_" + to_string(N) + ".txt";
...
// g++ -o 1D_EDP_Ch_ex1 1D_EDP_Ch_ex1.cpp && ./1D_EDP_Ch_ex1