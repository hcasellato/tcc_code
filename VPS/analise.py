...
def plot_heatmap_and_vector_field(df, output_path="campo_e_heatmap.png"):
    """Plot heatmap and vector field side by side
    ...
    try:
        ...
        # Plot heatmap da solução numérica
        sns.heatmap(pivot_test, ax=ax[0], cmap='viridis', vmin=-1, vmax=1)
        ax[0].set_title('Numerical Solution (Heatmap)')
        ax[0].set_xlabel('X Coordinate')
        ax[0].set_ylabel('Y Coordinate')

        # Plot campo vetorial numérico
        ax[1].quiver(
            df['x'], df['y'],
            df['Vx'], df['Vy'],
            color='red', angles='xy', scale_units='xy',
        ...
        ax[1].axis('equal')
        ax[1].set_xlim(df['x'].min(), df['x'].max())
        ax[1].set_ylim(df['y'].min(), df['y'].max())

        fig.suptitle("Numerical Solution and Vector Field", y=0.95)
        plt.tight_layout()
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
        print(f"Error plotting heatmap and vector field: {str(e)}")
        raise
...
if __name__ == "__main__":
    try:
        # Load data
        df = load_data("VPS_db/VPS_FVM_625.txt")
        
        # Generate visualizations
        plot_heatmaps(df)
        plot_vector_fields(df)
        plot_heatmap_and_vector_field(df)
