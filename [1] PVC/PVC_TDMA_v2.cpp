...
int main()
{
  ...
  */
  ...
  // Lembrando, para uma configuração Ax = d
  
  double diag_a[N-1], diag_b[N-1], diag_c[N-1];
  double malha_d[N+1];
  
  double diag_c_new[N-1], malha_d_new[N+1]; // valores novos para Alg. de Thomas
  ...
  for(int i = 1; i < N; i++)
    malha_d[i] = -1.0*pow(h,2)*r_function(malha_x[i]);

  // Aqui, o primeiro valor de a (o oculto) é passado ao lado direito da equação
  // lá naquela equação completa (Vanessa, pg 3)
  malha_d[1] -= diag_a[0]*sol_a;
  diag_a[0]   = 0;
  
  // Mesma coisa aqui
  malha_d[N-1] -= diag_c[N-2]*sol_b;
  diag_c[N-2]   = 0;
  
  // Resolver pelo Algoritmo de Thomas (também suporta paralelização)
  // Matriz da forma Ax = d
  ...
  malha_d_new[1] = malha_d[1] / diag_b[0];
  for(int i = 2; i < N; i++)
    malha_d_new[i] = (malha_d[i] - malha_d_new[i-1] * diag_a[i-1]) / (diag_b[i-1] - diag_c_new[i-2] * diag_a[i-1]);

  // Solução:
  malha_w[N-1] = malha_d_new[N-1];
  for(int i = N-2; i > 0; i--)
    malha_w[i] = malha_d_new[i] - diag_c_new[i-1] * malha_w[i+1];
...
// g++ -o PVC_TDMA_v2 PVC_TDMA_v2.cpp && ./PVC_TDMA_v2
