...
const double KRIGING_NUGGET = 1e-6;         // Stabilizes Kriging matrix

// Geological measurement point
struct Measurement {
  double x, y;
  double value; // ξ value at location (log permeability)
};

// Gaussian random field generator with Kriging
class GaussianField {
private:
    int size;            // Grid size (size x size)
    double beta;         // Covariance exponent
    double b_paper;      // Paper's covariance parameter
    double b_our;        // Scaled parameter for our field
    double dx;           // Grid spacing
    MatrixXd field;      // Generated field (ξ_our)
    VectorXd x_grid, y_grid;
    std::vector<Measurement> measurements;
    LLT<MatrixXd> llt;  // Cholesky decomp for Kriging

    // Scale factor for ξ: K = exp(4.5 * ξ_our)
    const double XI_SCALE = 4.5; 

public:
    GaussianField(int n, double b, double bet, 
                 const std::vector<Measurement>& measurements = {},
                 unsigned int seed = 1)
        : size(n), b_paper(b), beta(bet), measurements(measurements) 
    {
        dx = 1.0 / (size - 1);
        ...
        // Cov(ξ_paper) = (4.5)^2 * Cov(ξ_our) = b_paper * r^{-β}
        b_our = b_paper / (XI_SCALE * XI_SCALE);
        
        // Prepare Kriging system if measurements exist
        if (!measurements.empty()) setup_kriging();
        
        generate_field(seed);
    }

    // Set up Kriging covariance matrix
    void setup_kriging() {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                double r = distance(measurements[i], measurements[j]);
                C_mm(i, j) = covariance(r);
            }
            // Nugget effect for numerical stability
            C_mm(i, i) += KRIGING_NUGGET;
        }
        
        // Compute Cholesky decomposition
        llt = C_mm.llt();
        if (llt.info() != Success) {
            throw std::runtime_error("Kriging matrix not positive definite");
        }
    }

    // Covariance function: b_our * r^{-β}
    double covariance(double r) const {
        // Handle r=0 case (diagonal) and small distances
        if (r < 1e-8) return b_our / pow(dx, beta);
        return b_our * pow(r, -beta);
    }

    // Distance between points
    double distance(const Measurement& m1, const Measurement& m2) const {
        return std::hypot(m1.x - m2.x, m1.y - m2.y);
    }
    ...
    double kriging_interpolation(double x, double y, 
                                const VectorXd& data) const {
        int n = measurements.size();
        VectorXd c_x(n);
        
        // Covariance between (x,y) and measurement points
        for (int i = 0; i < n; ++i) {
            double r = distance(x, y, measurements[i].x, measurements[i].y);
            c_x(i) = covariance(r);
        }
        
        // Solve for weights: C_mm * weights = c_x
        VectorXd weights = llt.solve(c_x);
        return weights.dot(data);
    }

    // Generate constrained random field
    void generate_field(unsigned int seed) {
        // Create unconditional field
        MatrixXd uncond_field = generate_unconditional_field(seed);
        
        // If no measurements, use unconditional field
        if (measurements.empty()) {
            field = uncond_field;
            return;
        }

        // Kriging for deterministic part (ξ_d)
        VectorXd meas_data(measurements.size());
        for (int i = 0; i < measurements.size(); ++i)
            meas_data(i) = measurements[i].value / XI_SCALE;
        
        MatrixXd xi_d(size, size);
        for (int i = 0; i < size; ++i) {
            for (int j = 0; j < size; ++j) {
                xi_d(i, j) = kriging_interpolation(
                    x_grid(i), y_grid(j), meas_data
                );
            }
        }

        // Kriging for residual correction
        VectorXd resid_data(measurements.size());
        for (int i = 0; i < measurements.size(); ++i) {
            double x = measurements[i].x;
            double y = measurements[i].y;
            // Interpolate unconditional field at measurement points
            resid_data(i) = bilinear_interpolate(uncond_field, x, y);
        }
        
        MatrixXd correction_field(size, size);
        for (int i = 0; i < size; ++i) {
            for (int j = 0; j < size; ++j) {
                correction_field(i, j) = kriging_interpolation(
                    x_grid(i), y_grid(j), resid_data
                );
            }
        }

        // Construct final field: ξ_our = ξ_d + (ξ_uncond - correction)
        field = xi_d.array() + (uncond_field - correction_field).array();
    }

    // Generate unconditional Gaussian field
    MatrixXd generate_unconditional_field(unsigned int seed) {
        ...
        for (int i = 0; i < size; ++i) {
            for (int j = 0; j < size; ++j) {
                int idx = i * size + j;
                double kx = 2 * PI * (i <= size/2 ? i : i - size) / (dx * size);
                double ky = 2 * PI * (j <= size/2 ? j : j - size) / (dx * size);
                double k = std::hypot(kx, ky);
                
                // Power spectrum: P(k) = b_our * k^{-(2 - β)}
                double power = 0.0;
                if (k > 1e-8) {
                    power = b_our * std::pow(k, -(2 - beta));
                }
                
                // Complex Gaussian noise
                double real_part = dist(gen) * std::sqrt(power/2.0);
                double imag_part = dist(gen) * std::sqrt(power/2.0);
                in[idx][0] = real_part;
                in[idx][1] = imag_part;
            }
        }
        ...
        for (int i = 0; i < size; ++i) {
            for (int j = 0; j < size; ++j) {
                result(i, j) = out[i * size + j][0]; // Real part
            }
        }
        
        // Normalize to desired variance
        double mean = result.mean();
        MatrixXd centered = result.array() - mean;
        double var = centered.squaredNorm() / (size * size);
        double scale = std::sqrt(b_our / var);
    ...
    double bilinear_interpolate(const MatrixXd& grid, double x, double y) const {
        // Find grid indices
        int i = static_cast<int>(x / dx);
        int j = static_cast<int>(y / dx);
        i = std::clamp(i, 0, size - 2);
        j = std::clamp(j, 0, size - 2);
        
        // Interpolation weights
        double dx = x - x_grid(i);
        double dy = y - y_grid(j);
        double tx = dx / (x_grid(i+1) - x_grid(i));
        double ty = dy / (y_grid(j+1) - y_grid(j));
        
        // Bilinear interpolation
        return (1 - tx) * (1 - ty) * grid(i, j) +
               tx * (1 - ty) * grid(i+1, j) +
               (1 - tx) * ty * grid(i, j+1) +
               tx * ty * grid(i+1, j+1);
    }

    // Get permeability K = exp(4.5 * ξ_our)
    double get_permeability(double x, double y) const {
        return std::exp(XI_SCALE * interpolate(x, y));
    }

    // Interpolate ξ_our at arbitrary position
    double interpolate(double x, double y) const {
        return bilinear_interpolate(field, x, y);
    }

    // Access generated field
...
double K_function(GaussianField gf, double x, double y) {
  if(homogeneo == 0){
    return gf.get_permeability(x, y);
  } else {
    return 1.0;
  }
}
...
void FinVol(int debug_const)
{
  ...
  double beta = 0.5;     // Covariance exponent
  double b_paper = 1.0;  // Paper's covariance parameter

  std::vector<Measurement> measurements = {
    {0.2, 0.3, 0.8},  // (x, y, ξ_paper)
    {0.7, 0.5, -0.2}
  };

  // Convert measurements to ξ_our scale: ξ_paper = 4.5 * ξ_our
  for (auto& m : measurements) {
    m.value /= 4.5;  // Convert to ξ_our scale
  }

  GaussianField gf(M, b_paper, beta, measurements, 42);
...
// g++ -std=c++11 -O3 -I/usr/include/eigen3 -I/usr/include/ 2D_VFUW_H.cpp -lfftw3 -lm -o 2D_VFUW_H && ./2D_VFUW_H